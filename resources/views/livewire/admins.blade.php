<div class="p-6">
                                ...
                                @foreach ($data as $item)
                                    <tr>
                                        <td class="px-6 py-4 text-sm whitespace-no-wrap">
                                            {{ $item->title }}
                                            {!! $item->is_default_home ? '<span class="text-green-400 text-xs font-bold">[Default Home Admin]</span>':''!!}
                                            {!! $item->is_default_not_found ? '<span class="text-red-400 text-xs font-bold">[Default 404 Admin]</span>':''!!}
                                        </td>
                                        <td class="px-6 py-4 text-sm whitespace-no-wrap">
                                            <a
    ...
    <x-jet-dialog-modal wire:model="modalFormVisible">
        ...
        <x-slot name="content">
            ...
            {{-- <div class="mt-4">
                <label>
                    <input class="form-checkbox" type="checkbox" value="{{ $isSetToDefaultHomeAdmin }}" wire:model="isSetToDefaultHomeAdmin"/>
                    <span class="ml-2 text-sm text-gray-600">Set as the default home Admin</span>
                </label>
            </div>
            <div class="mt-4">
                <label>
                    <input class="form-checkbox" type="checkbox" value="{{ $isSetToDefaultNotFoundAdmin }}" wire:model="isSetToDefaultNotFoundAdmin"/>
                    <span class="ml-2 text-sm text-red-600">Set as the default 404 error Admin</span>
                </label>
            </div> --}}
            <div class="mt-4">
    ...
    <x-jet-dialog-modal wire:model="modalConfirmDeleteVisible">
        ...
        <x-slot name="content">
            {{ __('Are you sure you want to delete this admin? Once the admin is deleted, all of its resources and data will be permanently deleted.') }}
        </x-slot>
